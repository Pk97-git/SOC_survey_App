...
import { Text, Button, Surface, TextInput, useTheme, IconButton, Portal, Dialog, List, TouchableRipple } from 'react-native-paper';
...
export default function StartSurveyScreen() {
    ...
    const loadExistingSurveys = async () => {
        ...
        try {
            ...
            if (hybridStorage.syncService.isOnline) {
                try {
                    const backendSurveys = await surveysApi.getAll(siteId);
    ...
    const handleStartExistingSurvey = async (survey: any) => {
        ...
        try {
            // Update local status
            await storage.updateSurveyStatus(survey.id, 'in_progress');
    ...
    const createAndStartNewSurvey = async () => {
        try {
            ...
            // On Web, this returns the REAL backend survey (with a real ID). On Native it returns the newSurvey with mock ID.
            const savedSurvey = await hybridStorage.saveSurvey(newSurvey);
            const actualSurveyId = savedSurvey?.id || surveyId;
            ...
            >
                                ...
                                <Surface key={survey.id} style={styles.surveyRow} elevation={0}>
                                    <View style={{ flex: 1 }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6, marginBottom: 4 }}>
                                            ...
                                            <View style={[styles.badge, { backgroundColor: statusConfig.bg, borderWidth: 1, borderColor: statusConfig.border }]}>
                ...
                {siteName && serviceLine && (
                    <Surface style={[styles.infoBox, { backgroundColor: theme.colors.secondaryContainer }]} elevation={1}>
                        <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: theme.colors.secondary, justifyContent: 'center', alignItems: 'center' }}>
                            <MaterialCommunityIcons name="information" size={22} color={theme.colors.onSecondary} />
                        </View>
