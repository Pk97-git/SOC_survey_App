...
export default function ReportsScreen() {
    ...
    const handleGenerateReport = async (survey: any, options?: ExportOptions) => {
        try {
            ...
            // On web FileSystem.documentDirectory is null — let the browser handle the download
            let destination: string | undefined;
            if (Platform.OS !== 'web' && FileSystem.documentDirectory) {
                const siteName = survey.site_name ? survey.site_name.replace(/[^a-z0-9]/gi, '_') : 'Unknown_Site';
                const siteDir = `${FileSystem.documentDirectory}SavedReports/${siteName}`;
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            await hybridStorage.deleteSurvey(surveyId);
                            loadReports();
                ...
                <View style={styles.cardContent}>
                    ...
                    <View style={styles.surveyInfo}>
                        ...
                        <View style={[
                            styles.statusBadge,
                            {
                                backgroundColor: (Colors.surveyStatus[item.status] || Colors.surveyStatus.draft).bg,
                                borderWidth: 1,
                                borderColor: (Colors.surveyStatus[item.status] || Colors.surveyStatus.draft).border,
                            }
                        ]}>
                            <Text style={{
                                fontSize: 12,
                                fontWeight: '600',
                                color: (Colors.surveyStatus[item.status] || Colors.surveyStatus.draft).text,
                            }}>
    ...
    const handleBatchExport = async () => {
        ...
        try {
            // 1. Fetch assets for this specific site from backend
            const siteAssets = await hybridStorage.getAssets(selectedSite.id);
    ...
    const siteContextSurveys = selectedSite
        ? allSurveys.filter(survey => (survey.site_id === selectedSite.id) || (survey.site_name === selectedSite.name))
        : allSurveys;
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1, marginRight: 8 }}>
                        ...
                        <Text style={{ fontSize: 14, color: theme.colors.onSurfaceVariant, marginTop: 4, fontWeight: '500' }} numberOfLines={1}>
                            {selectedSite?.name || 'All Sites'} • {siteContextSurveys.length} surveys
                        </Text>
                    </View>
            ...
            <View style={styles.filterContainer}>
                <FlatList
                    ...
                    data={[
                        { label: 'All', value: 'all', count: siteContextSurveys.length },
                        { label: 'Submitted', value: 'submitted', count: siteContextSurveys.filter(s => s.status === 'submitted').length },
                        { label: 'In Progress', value: 'in_progress', count: siteContextSurveys.filter(s => s.status === 'in_progress').length },
                        { label: 'Draft', value: 'draft', count: siteContextSurveys.filter(s => s.status === 'draft').length },
                    ]}
